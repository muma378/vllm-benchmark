...
def pack_metadata(shape, dtype, size, fmt: MemoryFormat) -> bytes:
    metadata_desc = "<QQQQQ" + len(shape) * "Q"
    ...
    return struct.pack(
        metadata_desc, _METADATA_VERSION, dtype_to_idx[dtype], size, fmt.value, len(shape), *shape
    )


def unpack_metadata(buffer):
    version, dt_idx, size, fmt_val, ndim = struct.unpack_from("<QQQQQ", buffer)
    shape_offset = struct.calcsize("<QQQQQ")
    ...
    return torch.Size(shape), torch_dtypes[dt_idx], size, MemoryFormat(fmt_val)
...
class WekaGdsBackend(AllocatorBackendInterface):
    ...
    def _read_metadata(self, key, filename, subdir_key):
        ...
        shape, dtype, size, fmt = unpack_metadata(buf)
        logger.debug(
            f"Read metadata for {key} from {filename}: "
            f"shape={shape}, dtype={dtype}, size={size}, fmt={fmt}"
        )
        metadata = DiskCacheMetadata(
            filename.removesuffix(_METADATA_FILE_SUFFIX), size, shape, dtype, None, fmt
        )
        with self.hot_lock:
    ...
    def _try_to_read_metadata(self, key: CacheEngineKey) -> Optional[DiskCacheMetadata]:
        ...
        # Check if retry is enabled via extra config
        retry_enabled = False
        timeout = 2.0  # default timeout in seconds
        
        if self.config.extra_config is not None:
            gds_retry = get_extra_config_bool("gds_retry_to_read", self.config)
            if gds_retry is not None:
                retry_enabled = gds_retry
            
            # Allow configuring timeout via extra_config
            timeout_config = self.config.extra_config.get("gds_retry_timeout", None)
            if timeout_config is not None:
                try:
                    timeout = float(timeout_config)
                except ValueError:
                    logger.warning(
                        f"Invalid gds_retry_timeout value: {timeout_config}, "
                        f"using default {timeout}s"
                    )
        ...
        if retry_enabled:
            ...
            if found:
                logger.info(f"metadata for {path} comes alive in {end - start:.4f} seconds")
            else:
                logger.info(f"metadata for {path} NOT found after timeout {timeout} seconds")
    ...
    async def _async_load_bytes_from_disk(
        ...
        return self._load_bytes_from_disk_with_allocation(key, path, dtype, shape, fmt=fmt)
    ...
    def batched_get_blocking(
        ...
        for dtype, shape, path, fmt in zip(dtypes, shapes, paths, fmts, strict=True):
            if path is None:
                ...
                continue
            logger.info(
                    f"batched_get_blocking: dtype={dtype}, fmt={fmt}"
            )
    ...
    def _save_gds_cufile(
        ...
        metadata = pack_metadata(kv_chunk.shape, kv_chunk.dtype, kv_chunk.nbytes, fmt)
        try:
    ...
    def _load_gds_cufile(
        ...
        with self.cufile.CuFile(file_path, "r", use_direct_io=self.use_direct_io) as f:
            return f.read(
                gpu_pointer,
                size_in_bytes,
