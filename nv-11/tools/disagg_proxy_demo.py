...
class Proxy:
    ...
    async def add_instance_endpoint(self, request: Request):
        try:
            ...
            try:
                # Allow localhost, IP addresses, and hostnames/domain names
                if host != "localhost":
                    # Try to parse as IP address, but don't fail if it's a hostname
                    try:
                        ipaddress.ip_address(host)
                    except ValueError:
                        # It's not an IP, assume it's a valid hostname
                        # Basic hostname validation: not empty and contains valid characters
    ...
    async def create_completion(self, raw_request: Request):
        try:
            ...
            # Check if max_tokens is 1, if so, skip decode stage
            max_tokens = request.get("max_tokens", None)
            if max_tokens == 1:
                logger.info("max_tokens=1, skipping decode stage and using prefill result directly")
                prefill_instance = self.schedule(self.prefill_cycler)
                try:
                    generator = self.forward_request(
                        f"http://{prefill_instance}/v1/completions", request
                    )
                except HTTPException as http_exc:
    ...
    async def create_chat_completion(self, raw_request: Request):
        try:
            ...
            # Check if max_tokens or max_completion_tokens is 1, if so, skip decode stage
            max_tokens = request.get("max_tokens", None)
            max_completion_tokens = request.get("max_completion_tokens", None)
            
            if max_tokens == 1 or max_completion_tokens == 1:
                logger.info("max_tokens/max_completion_tokens=1, skipping decode stage and using prefill result directly")
                prefill_instance = self.schedule(self.prefill_cycler)
                try:
                    generator = self.forward_request(
                        f"http://{prefill_instance}/v1/chat/completions", request
                    )
                except HTTPException as http_exc:
                    self.remove_instance_endpoint("prefill", prefill_instance)
                    raise http_exc
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.0.0.0",
        help="Server host address (default: 0.0.0.0 to accept all connections)",
    )
